...
static u32 ana_write(u32 reg_addr,u32 value)
{
	...
	DEBUGSEE("%s: %d ctrl5:0x, reg_addr:0x%x val:0x%x",__FUNCTION__,__LINE__,reg_addr,value);
	close(fd_reg);
	close(fd_val);
	return ret;
}
...
int read_dcx_data(char *buff, int nLen)
{
	...
	if(NULL == buff)
	{
		DEBUGSEE("%s: res is NULL!!!",__FUNCTION__);
		ret = -1;
		return ret;
	}
	if(access(ENG_RAWDATA_FILE, F_OK) == 0) {
		DEBUGSEE("%s: %s exists",__FUNCTION__, ENG_RAWDATA_FILE);
		fp = fopen(ENG_RAWDATA_FILE, "r");
		if(NULL == fp)
		{
		  DEBUGSEE("%s: fopen fail errno=%d, strerror(errno)=%s",__FUNCTION__, errno, strerror(errno));
		  ret = -1;
		  return ret;
		}
		...
		DEBUGSEE("%s: fread count %d",__FUNCTION__, rcount);
		fclose(fp);
	}else{
		ret = -1;
		DEBUGSEE("%s: %s not exists",__FUNCTION__, ENG_RAWDATA_FILE);
		return ret;
	}
	return rcount;
}
