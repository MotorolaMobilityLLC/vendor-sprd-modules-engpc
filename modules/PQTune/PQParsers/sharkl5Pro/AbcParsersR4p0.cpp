...
static int parse_epf_child_config(abc_common_sharkl5Pro *abc, xmlNodePtr curNode , int epf_fun, int i)
{
	...
	while(NULL != subNode) {
		...
		while (NULL != propNode) {
			...
			if (propNode)
				propNode = propNode->next;
				ENG_LOG("aabbcc parsel5p5o %s  j = %d \n", __func__, j);
		}
	...
	while(NULL != subNode) {
		if(xmlHasProp(subNode, BAD_CAST "mode")) {
			...
			if(!xmlStrcmp(szPropity, (const xmlChar *) "normal")) {
				...
				subNode = FindNode(subNode->children, "number");
...
int AbcParserR4p0::parse_reg(uint08_t *ctx)
{
	...
	abc->sceneTable[0].sceneTableItem[0].slpCfg.first_pth_index2 = slp_ltm_params_l5pro.fst_pth_index[3];	
