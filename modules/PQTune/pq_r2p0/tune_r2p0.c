...
struct ops_entry tune_r2p0_entry;
...
static int tune_connect(char *buf, int len, char *rsp, int rsplen)
{
	...
	sizes = ptemp - pchar - 1;
	strncpy(dut_info->szModelName, tune_r2p0_entry.version, strlen(tune_r2p0_entry.version));
	...
	close(fd2);
	close(fd3);
	ENG_LOG("pq_cmd_connect sucess v1\n");
	return rsp_len;
}
...
static int tune_start_read_cfg(char *buf, int len, char *rsp, int rsplen)
{
	...
	rsp_head = (MSG_HEAD_T *)(rsp + 1);
	parse_r2p0_abc_xml(&(ctx.params.abc));
	parse_r2p0_cms_xml(&(ctx.params.cms));
	parse_r2p0_bld_xml(&(ctx.params.bld));
	parse_r2p0_gamma_xml(&(ctx.params.gamma));
...
static int tune_end_write_cfg(char *buf, int len, char *rsp, int rsplen)
{
	...
	if(ctx.offset == sizeof(ctx.params)) {
		update_r2p0_abc_xml(&ctx.params.abc);
		update_r2p0_cms_xml(&ctx.params.cms);
		update_r2p0_bld_xml(&ctx.params.bld);
		update_r2p0_gamma_xml(&ctx.params.gamma);
		err = 0;
	} else
		err = 1;
...
static int tune_wr_tuning_reg(char *buf, int len, char *rsp, int rsplen)
{
	...
	switch(*sub_cmd) {
		...
		case PQ_GAMMA:
			...
			update_r2p0_gamma_reg(&ctx.params.gamma);
			break;
		case PQ_BLD:
			sizes = sizeof(ctx.params.bld);
			memcpy((u8 *)&ctx.params.bld, pdata, sizes);
			update_r2p0_bld_reg(&ctx.params.bld);
			break;
		case PQ_CMS:
			sizes = sizeof(ctx.params.cms);
			memcpy((u8 *)&ctx.params.cms, pdata, sizes);
			update_r2p0_cms_reg(&ctx.params.cms);
			break;
		case PQ_ABC:
			sizes = sizeof(ctx.params.abc);
			memcpy((u8 *)&ctx.params.abc, pdata, sizes);
			update_r2p0_abc_reg(&ctx.params.abc);
...
static struct tune_ops tune_ops_r2p0 = {
