...
//Add for slog_bridge control
const char* Os_getBridgeCtlPath() {
if (0 == access(SYS_MODULE_SLOG_BRIDGE_TRANSPORT, F_OK)) {
        ALOGE("Engpc: %s: zachary bridge control path: %s", __FUNCTION__,SYS_MODULE_SLOG_BRIDGE_TRANSPORT);
        return SYS_MODULE_SLOG_BRIDGE_TRANSPORT;
    } else {
        ALOGE("Engpc: %s: fail to access SYS_MODULE_SLOG_BRIDGE_TRANSPORT ! errno = %d, strerror(errno) = %s",__FUNCTION__,errno,strerror(errno));
        return NULL;
    }
...
int Os_slog_bridge_enable() {
  ...
  int ret = 0;
    ...
    const char* path = Os_getBridgeCtlPath();
    if (path == NULL) {
        ALOGE("Engpc: %s: invalid slog bridge control path.",__FUNCTION__);
        return 0;
    }

    ALOGD("Engpc: %s: bridge control path: %s", __FUNCTION__,path);

    fd = open(path, O_WRONLY);
    if (fd >= 0){
        ret = write(fd, "1", 2);
         if (ret < 0) {
         ALOGE("Engpc: %s write %s failed! errno = %d, strerror(errno) = %s \n", __FUNCTION__, path,errno,strerror(errno));
         close(fd);
         return 0;
         }
        ALOGD("Engpc: %s: write %s success!\n", __FUNCTION__, path);
        close(fd);
        return 1;
    }else{
        ALOGE("Engpc: %s: fail to open %s errno = %d, strerror(errno) = %s",__FUNCTION__, path,errno,strerror(errno));
        return 0;
    }
