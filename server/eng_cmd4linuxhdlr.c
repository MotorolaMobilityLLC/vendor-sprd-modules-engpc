...
// 1: pc 2: t card
//
int cplogctrl_setlocation(char log_type, char location, int diagportswitch) {
  int ret = 0;
  eng_thread_t t1, t3;
...
/* AT+SPATCPLOG=[log_type],[log_dest],[diagport_switch]
* log_dest     : 0 - no log
*                1 - to pc
*                2 - to memory card
*
...
static int eng_linuxcmd_cplogctl(char *req, char *rsp) {
    ...
    if (strchr(req, '?') != NULL) {
        ...
        if(NULL != req && NULL != req+1) {
            req++;
            iDiagPortSwitch = *req-'0';
            ENG_LOG("%s: iDiagSwitch = %d\n", __FUNCTION__, iDiagPortSwitch);
        }

        ret = cplogctrl_setlocation(log_type_cmd[0], log_dest_cmd[0], iDiagPortSwitch);
        if(ret == -1) {
            goto out;
        }
