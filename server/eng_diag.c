...
#if (defined CONFIG_ENGPC_EFUSE) && (!defined CONFIG_MINIENGPC)
...
int isATcmd(char *buff)
{
    if (buff == NULL) return -1;
    if (strcasecmp(buff, "AT") == 0 || strcasecmp(buff, "AT\r") == 0 || strcasecmp(buff, "AT\n") == 0 || strcasecmp(buff, "AT\r\n") == 0){
        return 0;
    }
...
int isPPPDSucc(char *buff, char *rsp)
{
    if (buff == NULL || rsp == NULL) return -1;
    if (strcasecmp(buff, "ATD*99#\r") == 0 && strcasecmp(rsp, "\nCONNECT 296\r\n") == 0){
        return 0;
    }
...
int eng_diag_dymic_hdlr(unsigned char *buf, int len, char *rsp, int rsp_len) {
  ...
  unsigned char rtRsp[] = {0x7E, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x9C, 0x00, 0x0D, 0x0A, 0x7E};
  ...
  list_for_each(list_find,&eng_head){
    ...
    if ((buf[7] == 0x68) && (0 != strlen(modules_list->callback.at_cmd)) && (strcasestr(buf+9, modules_list->callback.at_cmd)) != NULL) { // at command
      ...
      if (isATcmd(modules_list->callback.at_cmd) == 0 && g_pppd_flag == 0) continue;
...
at_write:
    ...
    at_ret_error = ((strcasestr(rsp, "+CME ERROR:")) != NULL) ? 1 : 0;
    // write rsp to pc
    do {
    ...
    ENG_LOG("%s: Send data to serial, at_ret_error = %d\n", __FUNCTION__, at_ret_error);
    ...
    if (at_ret_error){
        eng_diag_write2pc(rtRsp, sizeof(rtRsp)/sizeof(unsigned char), get_ser_diag_fd());
    }else{
  ...
  list_for_each(list_find, &eng_head) {
    ...
    if ((0 != strlen(modules_list->callback.at_cmd)) &&
        ...
        if (NULL != modules_list->callback.eng_linuxcmd_func) {
          ...
          if (g_pppd_flag == 1 && isPPPDSucc(buf, rsp) == 0){
            ENG_LOG("%s: pppd succ...\n", __FUNCTION__);
            pppd = 1;
          }
...
at_write:
  // write rsp to pc
  ENG_LOG("%s: Send data to pc, rsp = %s\n", __FUNCTION__, rsp);
  if (NULL != writetopc_func) {
    writetopc_func(rsp, strlen(rsp));
  }


  //if pppd succ, then process exit
  if (g_pppd_flag == 1 && pppd == 1){
    ENG_LOG("%s: engpcclientpppd exit..\n", __FUNCTION__);
    exit(0);
  }
