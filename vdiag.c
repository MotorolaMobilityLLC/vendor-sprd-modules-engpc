...
static void print_log_data(int cnt) {
  ...
  if (cnt > DATA_BUF_SIZE / 2) cnt = DATA_BUF_SIZE / 2;
    ...
    do {
      ...
      if (modem_fd < 0) {
        ...
        if (wait_cnt++ >= MAX_OPEN_TIMES) {
          ENG_LOG(
              "eng_vdiag cannot open SIPC, try times exceed the max open "
              "times\n");
          close(ser_fd);
          return NULL;
        }
  ...
  while (1) {
    ...
    if (r_cnt <= 0) {
      ...
      do {
        ...
        if ((++wait_cnt) > MAX_OPEN_TIMES) {
          ENG_LOG(
              "eng_vdiag serial port open times exceed the max open times "
              "!!!\n");
          close(modem_fd);
          return NULL;
        }
    ...
    switch (g_diag_status) {
      ...
      case ENG_DIAG_RECV_TO_AP:
        if (get_user_diag_buf(log_data, r_cnt)) {
          ...
          init_user_diag_buf();  // complete diag framer, so AP decide the
                                 // following oper
          ...
          if (type != CMD_COMMON) {
            g_diag_status = ENG_DIAG_RECV_TO_AP;
            ENG_LOG(
                "%s: Buffer is full, so AP will not process the next package\n",
                __FUNCTION__);
    ...
    if (1 == has_processed || 1 == g_ap_cali_flag) {  // Data has been processed
                                                      // & should not send to
                                                      // modem
      backup_data_len = 0;
      continue;
